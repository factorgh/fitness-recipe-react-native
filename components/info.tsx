...
// //   return moment(date).utc().startOf("day").format("YYYY-MM-DD HH:mm:ss");
// };
export default function AllmealsTrainee({ date }: { date: any }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      const formattedDate = setToMidnight(date);
      console.log("<----FormattedDateTrainee----->", formattedDate);
      const token = await AsyncStorage.getItem("access_token");
      console.log("<-------tokenOnAllMeals-------->", token);
      try {
        const response = await axios.get(
          `${SERVER_URL}/api/v1/mealplans/trainee/${formattedDate}`,
          {
            headers: {
              "Content-Type": "application/json",
              Authorization: `${token}`,
